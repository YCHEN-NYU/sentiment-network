...
class SentimentNetwork:
    ...
    def init_network(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # initialize self.weights_0_1 as a matrix of zeros.(weights input->hidden layer)
        self.weights_0_1 = np.zeros((self.input_nodes, self.hidden_nodes), dtype=float)
        print('self.weights_0_1 = ', self.weights_0_1)
        # initialize self.weights_1_2 as a matrix of zeros.(weights hidden->output layer)
        self.weights_1_2 = np.random.normal(0.0, self.hidden_nodes**(-0.5), (self.hidden_nodes, self.output_nodes))
        # input layer layer_0, a two-dimensional matrix with shape (1, input_nodes), with all values initialized to zero
    ...
    def update_input_layer(self,review):
        ...
        # review here is a single block from the reviews
    ...
    def train(self, training_reviews, training_labels):
        ...
        for i in range(len(training_reviews)):
            ...
            hidden_inputs = np.dot(layer_0,  self.weights_0_1)
            ...
            hidden_error_term = hidden_errors*1.0
            ...
            self.weights_0_1 += self.learning_rate*self.layer_0[:, None]*hidden_error_term/1.0
            
            # TODO: Keep track of correct predictions. To determine if the prediction was
            #       correct, check that the absolute value of the output error 
            #       is less than 0.5. If so, add one to the correct_so_far count.
            if(abs(output_errors) < 0.5):
                correct_so_far += 1
            else:
                print('abs(output_errors) > 0.5')
